...
if (!empty(getenv('CODESPACE_NAME')) && !empty(getenv('GITHUB_CODESPACES_PORT_FORWARDING_DOMAIN'))) {
    // In Codespaces, use the external URL that users access
    $codespace_name = getenv('CODESPACE_NAME');
    $domain = getenv('GITHUB_CODESPACES_PORT_FORWARDING_DOMAIN');
    $GLOBALS['basesiteurl'] = "https://{$codespace_name}-80.{$domain}" . $imasroot;
} else {
